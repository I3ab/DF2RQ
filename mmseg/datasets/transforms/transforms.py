...
class MultimodalNormalize(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        Returns:
            ...
            result dict.
        """
        img_keys = [key for key in results.keys() if key.endswith("img")]
        for img_m in img_keys:
            mean_m = img_m.replace("img", "mean")
            std_m = img_m.replace("img", "std")
            results[img_m] = mmcv.imnormalize(
                results[img_m], 
                np.array(self.mean[mean_m], dtype=np.float32), 
                np.array(self.std[std_m], dtype=np.float32),
                self.to_rgb
            )
...
@TRANSFORMS.register_module()
class MultimodalResize(MMCV_Resize):
    """Resize images & seg & depth map.
...
@TRANSFORMS.register_module()
class MultimodalRandomResize(MMCV_RandomResize):
...
class MultimodalRandomFlip(RandomFlip):
    ...
    def _flip(self, results: dict) -> None:
        ...
        img_keys = [key for key in results.keys() if key.endswith("img")]
        for img_m in img_keys:
            results[img_m] = mmcv.imflip(
                results[img_m], direction=results['flip_direction'])

        img_shape = results[img_m].shape[:2]
...
class MultimodalRandomCrop(RandomCrop):     
    ...
    @cache_randomness
    def crop_bbox(self, results: dict, img_key: str) -> tuple:
        """get a crop bounding box.
    ...
    def transform(self, results: dict) -> dict:
        ...
        """
        img_keys = [key for key in results.keys() if key.endswith("img")]
        crop_bbox = self.crop_bbox(results, img_keys[0])
        
        # crop the image
        for img_m in img_keys:   
            results[img_m] = self.crop(results[img_m], crop_bbox)
        results['img_shape'] = results[img_m].shape[:2]

...
def preprare_mm_data(imgs, model: BaseModel):
    ...
    data['inputs']=dict()
    data_ = pipeline(imgs)
    img_keys = data_['inputs'].keys()
    for key in img_keys:
        data['inputs'][key]=[data_['inputs'][key]]
    data['data_samples']=[data_['data_samples']]

    return data

def mm_inference_model(model: BaseSegmentor,
    ...
    # prepare data
    data = preprare_mm_data(img, model)

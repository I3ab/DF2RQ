...
class LoadMultimodalImageFromFile(LoadImageFromFile):
    """Load Multimodal Remote Sensing Image from file.
    The modality of RSIs can be arbitrary.
    """
    def __init__(self, 
                 modality_keys: list[str]=['hsi', 'msi', 'sar'],
                 **kwargs):
        super().__init__(**kwargs)
        self.modality_keys = modality_keys

    def _fill_nan_mean(self, img):
        channel_means = np.nanmean(img, axis=(0, 1))
        if len(img.shape) > 2:
            for c in range(img.shape[-1]):
                channel_view = img[:, :, c]    
                nan_mask = np.isnan(channel_view)
                channel_view[nan_mask] = channel_means[c]
        else:
            nan_mask = np.isnan(img)
            img[nan_mask] = channel_means
        return img
    
    def _fill_nan_mini(self, img):
        img[np.isnan(img)] = -1.
        return img
    ...
    def transform(self, results: Dict) -> Dict:
        ...
        file_path_keys = [key for key in results.keys() if key.endswith("path")]
        img_keys = [path_key for path_key in file_path_keys if 'seg_map' not in path_key]
        img_keys =  [key for key in img_keys if key.split('_')[0] in self.modality_keys]
        for m in img_keys:
            modality = m.split('_')[0]
            filename = results[m]
            try:
                if self.imdecode_backend == 'gdal':
                    ds = gdal.Open(filename)
                    if ds is None:
                        raise Exception(f'Unable to open file: {filename}')
                    x = ds.ReadAsArray()
                    im_m = np.einsum('ijk->jki', x)
                else:
                    if self.file_client_args is not None:
                        file_client = fileio.FileClient.infer_client(
                            self.file_client_args, filename)
                        ...
                        img_bytes = fileio.get(
                            filename, backend_args=self.backend_args)
                    try:
                        img_m = mmcv.imfrombytes(
                            img_bytes, flag='unchanged', backend=self.imdecode_backend)
                    except PIL.UnidentifiedImageError:
                        img_m = mmcv.imfrombytes(
                            img_bytes, flag='unchanged', backend='tifffile')
            except Exception as e:
                if self.ignore_empty:
                    return None
                else:
                    raise e
            if np.isnan(img_m).all():
                img_m = self._fill_nan_mini(img_m)# in ndvi fill nan with -1 value
            if np.isnan(img_m).any():
                img_m = self._fill_nan_mean(img_m)
            if self.to_float32:
                img_m = img_m.astype(np.float32)
            results[modality + '_img'] = img_m
        
        # the spatial resolution (first 2 dim) of different img_keys should be same
        # NOTE this feature will influence the stack_batch and padding func in SegDataPreProcessor
        assert len(set([results[key.split('_')[0] + '_img'].shape[:2] for key in img_keys])) == 1 ,\
            "the spatial resolution of different modalities should be same"
        
        results['img_shape'] = img_m.shape[:2]
        results['ori_shape'] = img_m.shape[:2]
        del img_m
